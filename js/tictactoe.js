...
// Disable the stop button since it is not needed until game starts
window.onload = function() {watch()};
function watch() {
    var btn= document.getElementById("btnStop");
    btnDisabled(btn); // disable the stop button since game didnt start
}

// get 2 random numbers and store in an array to see who goes first
function rollForTurn() {
    ...
    // play dice sounds during roll for turn
    diceRoll();
    // build the string to show who rolled what
    for (i=0; i<xArray.length; i++) {
        var result = i + 1;
        var pOne = xArray[0];
        var pTwo = xArray[1];
        // hardcode in case of a tie
        if (pOne == pTwo) {
            pOne = 1;
            pTwo = 2;
        }
    ...
    return first;
} // end of roll for turn

/* ==============================================================================
   Start the game, roll for turn & determine the active player 
==================================================================================*/
function startGame() {
    var xTurn = 0;
...
// functions to style and disable/enable the game buttons 
...
// if user presses stop button - stop game and reset
function stopGame() {
    hideGameMsg(); 
    var btn = document.getElementById("btnStart");
    startEnabled(btn);
    ...
    // clear all the squares (move images out of view)
...
/* ==============================================================================
   right player config panel - checks the proposed avatar assignments and stops 
   them from being the same avatar
================================================================================= */
...
// determines and returns active players avatar
...
function avatarPlaced() {
    ...
    var showPlayer = document.getElementById("showPlayer");
    // check for winner
    if (parseText == "That's three in a row, Player 1 wins!" || parseText == "That's three in a row, Player 1 wins!") {
        showPlayer.innerHTML = "Game Stopped";
        showPlayer.style.color = "red";
    }
...
// check if current square available
...
function glowBoard(pos) {
	...
	for (var i=0; i<squares.length; i++) {
		if (i == index0) {
			...
			setTimeout(function() { bg1.style.backgroundColor = "rgb(215, 243, 247)"; }, 1100);
		} else if (i == index1) {
			var bg2 = squares[i];
			...
			setTimeout(function() { bg2.style.backgroundColor = "rgb(215, 243, 247)"; }, 1100);
		} else if (i == index2) {
			var bg3 = squares[i];
			...
			setTimeout(function() { bg3.style.backgroundColor = "rgb(215, 243, 247)"; }, 1100);
		}
	}
...
// make whole body background color change when win
function blink() {
...
/* ========================================
   Algorithms to find all possible wins
=========================================== */
// squares 012 - first row
function checkWinCon1(info,squareArray) {
	var winDetected = "on";
	var winCon1 = [0,1,2];	
	for (var i in info) {
		if (info[i].charAt(0) == "0") {
			var match0Avatar = info[i].charAt(1); //avatar
		}
...
function checkWinCon2(info,squareArray) {
	...
	for (var i in info) {
		if (info[i].charAt(0) == "3") {
			var match3Avatar = info[i].charAt(1); //avatar
		}
...
function checkWinCon3(info,squareArray) {
	...
	for (var i in info) {
		if (info[i].charAt(0) == "6") {
			var match6Avatar = info[i].charAt(1); //avatar
		}
...
function checkWinCon5(info,squareArray) {
	...
	for (var i in info) {
		if (info[i].charAt(0) == "1") {
			var match1Avatar = info[i].charAt(1); //avatar
		}
		if (info[i].charAt(0) == "4") {
			var match4Avatar = info[i].charAt(1);
		}
...
function checkWinCon6(info,squareArray) {
	...
	for (var i in info) {
		if (info[i].charAt(0) == "2") {
			var match2Avatar = info[i].charAt(1); //avatar
		}
		if (info[i].charAt(0) == "5") {
			var match5Avatar = info[i].charAt(1);
		}
...
/* ==============================================================
Fill square with right avatar if unoccupied when clicked by player
================================================================= */
function square1Animate() {
	...
	if (activePlayer != "Game Stopped") { 
		...
		if (verdict == undefined) { 
			var paintAvatar = determineAvatar(); // get right picture for whose turn it is
			...
			// add new selected square to list
...
// animations for X and O avatars
...
function animateX(selected) {
	selected.style.transform = (selected.style.transform == "translateY(-100%)" || null) ? "translateY(-100%)" : "translateY(-100%)";
}
